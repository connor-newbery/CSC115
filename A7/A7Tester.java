...
public class A7Tester {
	...
	static void hashmap_entry_test1() {
		...
		compare_lists (hash1_keys_order, hash1_values_order, hash1_size, l);//19
		displayResults(hm.size() == hash1_size, "number of entries");//20
...
///////////Testing Hastable Implementation////////////////////////////////////////////////////////////////////////
	public static void map1_test() {
		...
		displayResults(d.size() == 0 ,"size");//9
		d.put("abc", 10);

		displayResults(d.size() == 1,"put + size");//10
		try {
			int val = d.get("abc");

			displayResults(val == 10,"put + get");//11
		} catch (KeyNotFoundException e) {
			displayResults(false, "put + get");//11
		}

		d.put("abc", 15);
		displayResults(d.size() == 1,"put update + size");//12

		try {
			int val = d.get("abc");
			displayResults(val == 15,"put + get");//13
		} catch (KeyNotFoundException e) {
			displayResults(false, "put + get");//13
		}

		try {
			int val = d.get("def");
			displayResults(false, "put + get should not be there");//14
			val = val + 1;
		} catch (KeyNotFoundException e) {
			displayResults(true, "put + get should not be there");//14
		}

		boolean isThere = d.containsKey("abc");
		displayResults(isThere,"put + contains there");//15

		isThere = d.containsKey("def");
		displayResults(!isThere,"put + contains not there");//16
	...
	public static void map2_test() {
		...
		displayResults(d.size() == 10000, "put + size");//17
